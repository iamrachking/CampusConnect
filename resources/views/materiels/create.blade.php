                    ...
                    <form method="POST" action="{{ route('materiels.store') }}">
                        ...
                        {{-- <div class="mb-6">
                            ...
                            <select name="type_materiel" id="type_materiel" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                                ...
                                <option value="Vidéoprojecteur" {{ old('type_materiel') === 'Vidéoprojecteur' ? 'selected' : '' }}>Vidéoprojecteur</option>
                                <option value="Ordinateur" {{ old('type_materiel') === 'Ordinateur' ? 'selected' : '' }}>Ordinateur</option>
                                <option value="Tablette" {{ old('type_materiel') === 'Tablette' ? 'selected' : '' }}>Tablette</option>
                                <option value="Écran" {{ old('type_materiel') === 'Écran' ? 'selected' : '' }}>Écran</option>
                                <option value="Audio" {{ old('type_materiel') === 'Audio' ? 'selected' : '' }}>Équipement audio</option>
                                <option value="Autre" {{ old('type_materiel') === 'Autre' ? 'selected' : '' }}>Autre</option>
                            </select>
                        </div> --}}
