<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('reservations.show', $reservation) }}" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded">
                Retour
            </a>
        </div>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('reservations.update', $reservation) }}">
                        ...
                        <div class="mb-6">
                            ...
                            <select name="item_type" id="item_type" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                                ...
                                <option value="salle" {{ old('item_type', $reservation->item_type === 'App\\Models\\Salle' ? 'salle' : '') === 'salle' ? 'selected' : '' }}>Salle</option>
                                <option value="materiel" {{ old('item_type', $reservation->item_type === 'App\\Models\\Materiel' ? 'materiel' : '') === 'materiel' ? 'selected' : '' }}>Matériel</option>
                            </select>
                        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (currentType) {
                loadItems(currentType, itemSelect, '{{ $reservation->item_id }}');
            }
        });
        ...
        document.getElementById('item_type').addEventListener('change', function() {
            ...
            const itemSelect = document.getElementById('item_id');
            loadItems(type, itemSelect);
        });

        function loadItems(type, itemSelect, selectedId = null) {
                    ...
                    .then(data => {
                        ...
                        data.forEach(salle => {
                            if (salle.disponible) {
                                ...
                                if (selectedId && salle.id == selectedId) {
                    ...
                    .then(data => {
                        ...
                        data.forEach(materiel => {
                            if (materiel.disponible) {
                                ...
                                if (selectedId && materiel.id == selectedId) {
                                    option.selected = true;
                                }
                                itemSelect.appendChild(option);
                            }
