<x-clean-layout>
    <div class="bg-white/85 backdrop-blur rounded-xl shadow p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Nouvelle demande de réservation</h2>

        <form method="POST" action="{{ route('clean.teacher.store') }}" class="space-y-4">
            @csrf
            @if ($errors->any())
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('clean.teacher.index') }}" class="px-4 py-2 rounded bg-gray-200 text-gray-900 hover:bg-gray-300">Retour</a>
            </div>
        </form>
    </div>
</x-clean-layout>