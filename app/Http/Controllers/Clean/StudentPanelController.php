...
class StudentPanelController extends Controller
{
    ...
    {
        $user = request()->user();
        if (!$user || !$user->role || $user->role->name !== 'Étudiant') {
            abort(403, 'Accès interdit');
        }

        $salles = Salle::orderBy('nom_salle')->get();
        $materiels = Materiel::orderBy('nom_materiel')->get();
        return view('clean.student.availability', compact('salles', 'materiels'));
    }
    ...
    public function create()
    {
        ...
        $salles = Salle::orderBy('nom_salle')->get();
        $materiels = Materiel::orderBy('nom_materiel')->get();
        return view('clean.student.create', compact('salles', 'materiels'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('clean.student.availability')->with('status', 'Demande de réservation envoyée');
    }
}